          ...
          <nav className="flex items-center gap-3">
            ...
            >
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M9.5 6H2.5M5.5 2.5 2 6l3.5 3.5" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
              Back
            </Link>
          </nav>
      ...
      <main className="flex-1 max-w-5xl mx-auto px-6 w-full">
        ...
        <section className="pt-20 pb-16 border-b border-[#eef1f6] dark:border-white/[0.06]">
          ...
          <p className="text-base text-[#667085] dark:text-white/50 max-w-md leading-relaxed">
            Configure global defaults that apply across all projects and pages.
          </p>
        </section>
        ...
        <section className="py-12">
          ...
          <div className="mb-3">
            <p className="text-sm font-semibold text-[#101828] dark:text-white mb-1">Logo</p>
            <p className="text-xs text-[#667085] dark:text-white/40">
              Affects the logo displayed across all pages.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-[#eef1f6] dark:bg-white/[0.06] border border-[#eef1f6] dark:border-white/[0.06] rounded-xl overflow-hidden">
            {options.map((opt) => {
              ...
              return (
                <button
                  key={opt.id}
                  onClick={() => updateSettings({ logoMode: opt.id })}
                  className={`group bg-white dark:bg-[#080d14] p-7 flex flex-col gap-5 text-left transition-colors ${
                    active
                      ? 'bg-[#f0f6ff] dark:bg-[#0063F5]/[0.06]'
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className={`text-sm font-semibold mb-0.5 transition-colors ${active ? 'text-[#0063F5]' : 'text-[#101828] dark:text-white'}`}>
                        {opt.label}
                      </p>
                      <p className="text-xs text-[#667085] dark:text-white/40">{opt.description}</p>
                    </div>
                    <div className={`w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center transition-colors ${
                      active
                        ? 'border-[#0063F5] bg-[#0063F5]'
                        : 'border-[#d0d5dd] dark:border-white/20'
                    }`}>
                      {active && (
...
// Static preview — does not read from context so each card shows its own mode
function LogoPreview({ mode }: { mode: LogoMode }) {
  const icon = (
    <svg width="28" height="28" viewBox="0 0 34.3273 34.3273" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="16.6912" height="4.409" transform="matrix(-1 0 0 1 25.509 8.818)" fill="#0063F5" />
      <path d="M19.2107 8.818H14.8017V29.918L19.2107 34.327V8.818Z" fill="#0063F5" />
      <path d="M14.8016 29.918L19.2106 34.327L29.918 34.327L34.327 29.918L14.8016 29.918Z" fill="#004FC6" />
      <path d="M29.917 0L34.326 4.409V29.918L29.917 34.327V0Z" fill="#0063F5" />
      <path d="M0 4.409L4.409 0L29.918 0L34.327 4.409L0 4.409Z" fill="#004FC6" />
      <path d="M4.41 0L0.001 4.409V29.918L4.41 34.327V0Z" fill="#0063F5" />
    </svg>
  );
