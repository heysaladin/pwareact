...
const checkboxItems = [
  {
    required: true,
    text: 'I consent to CEER and its authorized financing partner obtaining my SIMAH credit report for the purpose of assessing my financing eligibility.',
  },
  {
    required: true,
    text: 'I authorize the use of my identity and verified mobile information for this financing journey.',
  },
  {
    required: true,
    textNode: (
...
export default function SimahConsentPage() {
  const [checked, setChecked] = useState([false, false, false, false]);

  const toggle = (i: number) => setChecked(prev => prev.map((v, idx) => idx === i ? !v : v));
  const allRequired = REQUIRED.every(i => checked[i]);
      ...
      <div className="flex flex-1">
        ...
        <div className="flex-1 px-10 py-8 overflow-y-auto">
              ...
              <div className="flex flex-col gap-3">
                {checkboxItems.map((item, i) => (
                  <div
                    key={i}
                    onClick={() => toggle(i)}
                    className="flex items-start gap-4 p-4 bg-[#0C0C0C] rounded-xl cursor-pointer hover:bg-white/[0.02] transition-colors select-none"
                  >
                    <div className={`w-5 h-5 rounded flex-shrink-0 mt-0.5 flex items-center justify-center transition-colors ${
                      checked[i] ? 'bg-[#E87722]' : 'border border-white/20 bg-transparent'
                    }`}>
                      {checked[i] && (
                    ...
                    <p className="text-xs text-white/70">
                      {item.required && <span className="text-[#E87722]">* </span>}
                      {item.text ?? item.textNode}
                    </p>
                  </div>
                ))}
              </div>
          ...
          <div className="mt-6 flex items-center justify-between">
            <span className="text-xs text-white/40 flex items-center gap-1 cursor-pointer hover:text-white/60 transition-colors">
            ...
            <div className="flex flex-col items-end gap-1">
              {allRequired ? (
                <Link
                  href="/ceer/compare-offers"
                  className="bg-[#E87722] text-white font-bold px-8 py-3.5 rounded-xl text-sm transition-opacity hover:opacity-90"
                >
                  Agree &amp; Continue →
                </Link>
              ) : (
              ...
              <span className="text-[11px] text-white/40">› Next: Compare financing offers</span>
            </div>
          </div>
