...
export default function CeerSidebar({ backHref, backLabel = 'Back to CEER' }: CeerSidebarProps) {
  return (
    <aside className="w-[350px] shrink-0 bg-[#0b1420] border-r border-[#2a3a4f] flex flex-col p-6 gap-0 sticky top-0 h-full overflow-y-auto">

      {/* Header */}
      <div className="flex items-center justify-between py-2">
        <svg width="100" height="14" viewBox="0 0 100 14" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-white">
          <g clipPath="url(#ceer-sb)">
          ...
          <defs><clipPath id="ceer-sb"><rect width="99.6471" height="14" fill="white"/></clipPath></defs>
        </svg>
        {backHref && (
      ...
      <div className="mt-3.5 bg-[#121e2e] border border-[#2a3a4f] rounded-2xl p-[21px] flex flex-col">
        <div
          className="w-full h-[180px] rounded-xl relative overflow-hidden"
          style={{ background: 'radial-gradient(ellipse at 50% 160%, rgba(37,63,98,1) 25%, rgba(28,47,72,1) 62%, rgba(18,30,46,1) 100%)' }}
        >
        ...
        <p className="mt-1 text-sm text-[#93a4b8]">Premium AWD – 2026</p>

        <div className="mt-3.5 border-t border-[#2a3a4f] pt-1.5 flex flex-col">
          {[
            ['🎨 Exterior', 'Quantum Grey'],
            ['💺 Interior', 'Onyx Black'],
            ['✳️ Wheels', '21" Alloy Sport'],
            ['🔋 Battery', '108 kWh'],
            ['📏 Range', '610 km'],
          ].map(([label, value]) => (
        ...
        <div className="mt-2 border-t border-[#2a3a4f] pt-3.5 flex flex-col">
          <div className="flex justify-between py-1.5 text-[13px]">
            <span className="text-[#93a4b8]">Vehicle price (incl. VAT)</span>
            <span className="text-[#e6edf5] font-semibold">SAR 228,750</span>
          </div>
          <div className="flex justify-between py-1.5 text-[13px]">
            <span className="text-[#93a4b8]">Down payment (20%)</span>
            <span className="text-[#e6edf5] font-semibold">SAR 45,750</span>
          </div>
        </div>

        <div className="mt-3.5 bg-[#16283f] rounded-[10px] px-4 py-3 flex items-center justify-between">
          <span className="text-[13px] text-[#e6edf5]">Amount to finance</span>
          <span className="text-[15px] font-bold text-[#e6edf5]">SAR 183,000</span>
        </div>

        <div className="flex justify-between pt-4 pb-1.5 text-[13px]">
          <span className="text-[#93a4b8]">Tenure</span>
          <span className="text-[#e6edf5] font-semibold">60 Months</span>
        </div>

        <div className="mt-2.5 bg-[#00265f] border border-[#0041a3] rounded-[10px] px-[17px] py-[13px] flex items-center justify-between">
          <span className="text-[12px] text-white">Total payable (SNB)</span>
          <span className="text-[15px] font-bold text-white">SAR 218,880</span>
        </div>
      </div>
      ...
      <div className="mt-3 bg-[#121e2e] border border-[#2a3a4f] rounded-xl p-[15px] flex gap-3 items-center">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[13px] font-bold text-[#e6edf5]">Your data is secure</p>
          <p className="text-[12.5px] text-[#93a4b8] mt-0.5">We use bank-level encryption to protect your information.</p>
        </div>
      </div>
      ...
      <div className="mt-3 bg-[#121e2e] border border-[#2a3a4f] rounded-xl p-[15px] flex gap-3 items-center">
        ...
        <p className="flex-1 text-[12.5px] text-[#93a4b8] leading-[18.75px]">Verification powered by Nafath — government-trusted identity services.</p>
      </div>
