'use client';
import { BarChart2, Calendar, Settings2, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="flex items-center justify-between w-full">
      ...
      <div className="flex items-center gap-2">
        ...
        <button className="flex items-center gap-1 px-3.5 py-2.5 bg-white border border-[#d5d7da] rounded-lg text-sm font-medium text-[#414651] shadow-[0px_1px_2px_0px_rgba(16,24,40,0.05)] hover:bg-gray-50 transition-colors dark:bg-slate-900 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
          <Settings2 className="w-5 h-5 text-[#667085] dark:text-slate-400 shrink-0" />
          <span>{isAr ? 'مؤشرات الأداء' : 'KPIs'}</span>
          <ChevronDown className="w-4 h-4 text-[#667085] dark:text-slate-400 shrink-0" />
        </button>
      </div>
    </div>
  );
