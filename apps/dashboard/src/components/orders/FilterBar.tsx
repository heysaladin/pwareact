...
import { TrendingUp, Building2, Search, X, ListFilter } from 'lucide-react';
...
function FlowItemCard({
  ...
  return (
    <div className="flex-1 min-w-0 bg-white border border-[#e3e8ef] rounded-[12px] flex flex-col gap-3 h-[210px] overflow-hidden pt-2 px-2 dark:bg-slate-900 dark:border-slate-700">
      <div className="flex items-center justify-between px-3 py-1 shrink-0">
        ...
        {hasSelection && (
          <button
            onClick={onClear}
            className="flex items-center gap-1 px-2.5 py-1 rounded-lg border border-[#d5d7da] bg-white text-[#414651] text-[12px] font-medium hover:bg-gray-50 transition-colors dark:bg-slate-800 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700 shrink-0"
          >
            <X className="w-3 h-3" />
            {isAr ? 'مسح' : 'Clear'}
          </button>
    ...
    <button
      ...
      className={cn(
        ...
        selected
          ...
          : 'border-[#e9eaeb] bg-transparent hover:bg-gray-50 dark:border-slate-700 dark:hover:bg-slate-800'
      )}
    >
...
export default function FilterBar() {
  ...
  const [selectedStatus,   setSelectedStatus]   = useState<string | null>('servicing');
  ...
  return (
    <div className="bg-[#fcfcfd] border-l border-r border-t border-[#e3e8ef] flex flex-col gap-4 p-4 rounded-tl-xl rounded-tr-xl w-full dark:bg-slate-900/50 dark:border-slate-700">
      <p className="text-[15px] font-semibold text-[#181d27] dark:text-slate-100 leading-[26px] shrink-0">
        {isAr ? 'رؤى سريعة' : 'Quick Insights'}
      </p>

      <div className="flex gap-4 w-full">
        {/* Order Statuses */}
        <FlowItemCard
          title={isAr ? 'حالات الطلبات' : 'Order Statuses'}
          icon={ListFilter}
          hasSelection={selectedStatus !== null}
        ...
        {/* Product Types */}
        <FlowItemCard
          title={isAr ? 'أنواع المنتجات' : 'Product Types'}
          icon={TrendingUp}
        ...
        >
          <div className="flex flex-col gap-1.5">
            <div className="relative mb-0.5">
              <Search className="absolute start-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#9aa4b2] dark:text-slate-500 pointer-events-none" />
              <input
                value={providerSearch}
                onChange={(e) => setProviderSearch(e.target.value)}
                placeholder={isAr ? 'بحث...' : 'Search provider...'}
                className="w-full h-8 ps-8 pe-3 border border-[#d5d7da] rounded-lg text-xs bg-white placeholder:text-[#697586] focus:outline-none focus:ring-1 focus:ring-[#0063f5] dark:border-slate-700 dark:bg-slate-900 dark:placeholder:text-slate-500 dark:text-slate-100"
              />
            </div>
            {filteredProviders.map((item) => (
              ...
              >
                {item.logo
                  ? <img src={item.logo} alt={item.label} className="h-4 w-12 shrink-0 object-contain" />
                  : <span className="text-[11px] font-semibold text-[#697586] dark:text-slate-400 w-12 shrink-0">{item.label}</span>
                }
              </DataRow>
            ))}
          </div>
